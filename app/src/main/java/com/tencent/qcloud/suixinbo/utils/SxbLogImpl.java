...
public class SxbLogImpl {
    ...
    private static MyLinkedBlockingDeque<String> logDeque = new MyLinkedBlockingDeque<String>(15000);
    ...
    private static String nowUsedFile = "";
    ...
    private static long lastWriterErrorTime = 0;
    ...
    static synchronized void initLogFile(long nowCurrentTimeMillis) throws IOException {
        logPath = Environment.getExternalStorageDirectory().getPath() + "/tencent/sxblog/"
                + packageName.replace(".", "/")
                + "/";
        ...
        try {
            ...
            if (!tmpeFile.exists()) {
                ...
                if (null != writer) {
                    writer.write(logTime + "|" + "|D|" + android.os.Build.MODEL + " "
                            + android.os.Build.VERSION.RELEASE + " create newLogFile "
                            + tmpeFile.getName() + " " + b + "\n");
                    writer.flush();
                }
            } else {
                if (null != writer) {
                    writer.write(logTime + "|" + "|E|" + android.os.Build.MODEL + " "
                            + android.os.Build.VERSION.RELEASE + "|newLogFile "
                            + tmpeFile.getName() + " is existed.\n");
                    writer.flush();
                }
            }
        } catch (Throwable e) {
            ...
            new Thread("QLVBLogInitThread") {
                ...
                public void run() {
                    try {
                        ...
                        System.out.println("QLog init post retry " + times
                                + " times, interval " + INTERVAL_RETRY_INIT[times]);
                        retryInitHandler.removeCallbacks(initRunnable);
                        retryInitHandler.postDelayed(initRunnable,
                                INTERVAL_RETRY_INIT[times] * 60000);
                        times++;
    ...
    public static void writeLog(String level, String tag, String msg, Throwable tr) {
        ...
        String message = logTime + "|" + level + "|" + String.valueOf(threadId)
                + "|" + tag + "|" + msg + "\n";
        if (null != tr) {
